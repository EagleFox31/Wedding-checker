...
import { TimelineItem, UserRole } from '../types';
import { CheckCircle2, Circle, Clock, Edit2, Trash2, Plus, AlertCircle, Timer, Play, Flag, RotateCcw, StopCircle, X, AlertTriangle } from 'lucide-react';
...
const PlanningView: React.FC<PlanningViewProps> = ({ items, userRole, onToggle, onEdit, onDelete, onAdd, onRefresh }) => {
  ...
  const handleStartParty = () => {
      // 1. Action Immédiate (Optimistic UI)
  ...
  // Find the index of the active item (first uncompleted)
  const activeIndex = items.findIndex(i => !i.completed);
  ...
  return (
    <div className="pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white p-4 sticky top-0 z-20 border-b border-slate-100 shadow-sm">
        <div className="flex justify-between items-end mb-2">
            ...
            <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">{completedCount}/{totalCount}</span>
            </div>
        </div>
      ...
      <div className="px-4 py-3 mt-4 bg-slate-100/50 mb-4 rounded-lg mx-4 flex items-center gap-3 border border-slate-100">
        <Clock className="text-slate-400" size={20} />
        <div>
           <h3 className="font-bold text-slate-800 text-sm">Programme de la soirée</h3>
           <p className="text-[10px] text-slate-500">
              {now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              {isPartyStarted ? ' • Suivi en cours' : ' • En attente de démarrage'}
           </p>
        </div>
      </div>

      {!loadingStatus && !isPartyStarted && (
          <div className="px-4 mb-6 animate-in zoom-in duration-300">
              <button 
                onClick={handleStartParty}
                className="w-full py-4 bg-slate-900 text-white rounded-xl shadow-lg shadow-slate-200 flex items-center justify-center gap-2 hover:bg-slate-800 active:scale-95 transition-all group"
              >
                  <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                     <Play size={16} fill="currentColor" />
                  </div>
                  <div className="text-left">
                    <span className="block font-bold text-sm leading-tight">LANCER LA SOIRÉE</span>
                    <span className="block text-[10px] text-slate-400 font-normal">Active le suivi en direct</span>
                  </div>
              </button>
          </div>
      )}
      ...
      <div className="relative px-4">
        ...
        {items.map((item, index) => {
           ...
           // Active status only visible if party started
           const isActive = isPartyStarted && index === activeIndex;
           ...
           if (isActive) {
               containerClass = "bg-white border-indigo-200 shadow-md ring-1 ring-indigo-50";
           ...
           return (
            <div 
                key={item.id} 
                className={`relative mb-6 pl-10 group transition-all duration-500`}
            >
                ...
                <button
                    ...
                    className={`
                        ...
                        ${isCompleted 
                            ...
                            : isActive 
                                ? 'border-indigo-500 text-indigo-500 ring-4 ring-indigo-50' 
                                : 'border-slate-300 text-transparent'
                        }
                    `}
                >
                    {isCompleted ? <CheckCircle2 size={16} fill="currentColor" className="text-white" /> : <div className={`w-2 h-2 rounded-full ${isActive ? 'bg-indigo-500' : 'bg-transparent'}`} />}
                </button>
                ...
                <div className={`p-4 rounded-xl border transition-all ${containerClass}`}>
                    <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-col items-start gap-1">
                            <span className={`text-xs font-bold tracking-wide px-2 py-0.5 rounded-full ${item.isHighlight && !isCompleted ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>
                                {item.time}
                            </span>
                            {/* Mobile Punctuality Badge */}
                            {isActive && delayBadge && (
                        ...
                        {isAdmin && (
                            <div className="flex gap-2">
                                <button onClick={() => onEdit(item)} className="p-1 text-slate-400 hover:text-indigo-600">
                                    <Edit2 size={14} />
                                </button>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 px-4">
            <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-5 animate-in zoom-in-95 duration-200">
                <div className="flex flex-col items-center text-center">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 mb-2">Réinitialiser la soirée ?</h3>
                    <p className="text-sm text-slate-500 mb-6">
                        Cela arrêtera le mode "Direct" et <b>décoche toutes les étapes</b> du planning. Cette action est irréversible.
                    </p>
